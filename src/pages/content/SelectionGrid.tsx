        ...
        <div className="w-full max-w-3xl mx-auto p-3">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 p-2">
            {selectedTokuTitles.map((item, index) => (
              <div
                key={item?.id || index}
                className="relative flex flex-col items-center justify-center aspect-square border border-gray-400 rounded-xl cursor-pointer hover:border-sky-400 hover:bg-sky-50 transition-all shadow overflow-hidden backdrop-blur-sm duration-300"
                onClick={() => !item && onOpenModal(index)}
              >
                <span className="absolute top-0 left-0 text-[10px] sm:text-sm font-bold text-white bg-sky-700/90 px-2 py-1 rounded-br-xl z-30 shadow transition-shadow">
                  {index + 1}
                </span>
                {item ? (
                  <>
                    <button
                      ...
                      className="absolute top-0 right-0 z-40 bg-red-500/90 hover:bg-red-600 text-white w-7 h-7 rounded-bl-xl flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-75 shadow no-export cursor-pointer"
                    >
